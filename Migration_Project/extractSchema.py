...
def extractSchema(cursor,schema):
  ...
  print(f"{bcolor.bcolors.WARNING}Following tables retrieved: {tables} {bcolor.bcolors.ENDC}")

  tNo = getTableNumber(tables)

  pks = fetch_all_PKs(cursor,tables)
  print(f"{bcolor.bcolors.WARNING}Following primary keys retrieved: {pks} {bcolor.bcolors.ENDC}")

  relations = fetch_relations(cursor,tables, schema)
  print(f"{bcolor.bcolors.WARNING}Following table relations retrieved: {relations} {bcolor.bcolors.ENDC}")
